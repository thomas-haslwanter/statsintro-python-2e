...
def positionOnScreen() -> None:
    """Position two plots on your screen. This uses the Tickle-backend, which
    I think is the default on all platforms."""
    ...
    def positionFigure(figure, geometry) -> None:
...
def on_key_event(event) -> None:
    ...
    if key in 'aeiou':
        ...
        curAxis.set_title(f'{key} is not a vowel: try again ....')
        plt.draw()
